...
export class RayLibHeader extends QuickJsHeader {
    ...
    private canConvertJsToC(type: string) {
        switch (type) {
            case "const void *":
            case "void *":
            case "float *":
            ...
            case "bool":
                return true
            default: {
                const key = type.replace("const ", "").replace(" *", "")
                return !!this.structLookup[key]
            }
        }
    }

    private canConvertCToJs(type: string) {
        switch (type) {
            case "int":
            case "long":
            case "unsigned int":
    ...
    addApiFunction(api: RayLibFunction){
        ...
        if (!options.body) {
            ...
            const unsupportedParam = activeParams.find(x => {
                if (x.binding?.customConverter) return false
                return !this.canConvertJsToC(x.binding?.typeAlias ?? x.type)
            })
            if (unsupportedParam) {
                console.warn(`Skipping function ${api.name}: unsupported parameter type ${unsupportedParam.type}`)
                return
            }
            if (api.returnType !== "void" && !this.canConvertCToJs(api.returnType)) {
                console.warn(`Skipping function ${api.name}: unsupported return type ${api.returnType}`)
                return
            }
        }
